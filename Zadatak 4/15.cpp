...
void unos(fudbaler** niz, int red, int kolona) {
	...
	for (int i = 0; i < red; i++)
	{
		for (int j = 0; j < kolona; j++)
		{
			...
			// Radi lakseg testiranja (Odkomentiraj za rucni unos): 
...
void prosjecan_broj_golova_Po_poziciji(fudbaler** niz, int red, int kolona) {
	...
	// Posto ne znamo koliko je odredjenih pozicija, moramo imati i brojac za svaku poziciju,
	// koji ce se povecavati svakom dodanom pozicijom.
	int brojacO = 0, brojacV = 0, brojacN = 0;
	// Moramo imati pomocne varijable gdje cemo smjestati prosjeke za svaku poziciju;
	// Posto se na njih sabira moraju biti inicijalizirane na 0 !
	...
	for (int i = 0; i < red; i++)
	{
		...
		// Za svaki brojac moramo provjeriti da li je razlicit od 0 i samo ako jeste tek onda izvrsiti djeljenje,
		// jer djeljenje sa 0 nema smisla(DOBIJETE REZULTAT TIPA: -nand, sto znaci not a number);
		if (brojacO != 0)
...
void najbolji_Klub_odabrane_Pozicije(fudbaler** niz, int red, int kolona) {
	...
	// Posto se od nas trazi da uzmemo unos odabrane pozicije od korisnika,
	// potrebno je napraviti novu varijablu tipa char, jer ona drzi 'O','V' ili 'N';
	char izbor;
	...
	// Posto se od nas trazi da nadjemo indeks najboljeg reda(kluba) sa korisnicki odabranom pozicijom,
	// prvo moramo napraviti pomocni niz redova tipa float u koji ce se smjestati svi prosjeci
	// {} - inicijalizirano sa 0;
	float* prosjeci_Odabranih_pozicija = new float[red] {};

	// Posto ne znamo koliko ce korisnicki odabranih pozicija biti u nasim redovima(klubovima)
	// moramo imati brojac koji ce se povecavati za svaku pronadjenu poziciju;
	int brojac = 0;
	...
	for (int i = 0; i < red; i++)
	{
		for (int j = 0; j < kolona; j++)
		{
			// Ako je trenutni element (igrac) iste pozicije kao i ona koju je korisnik unio, dodaj ga na sumu i inkrementuj brojac;
		...
		// Ponovo moramo provjeriti da li je brojac razlicit od nule te samo ako jeste onda dijeliti jer djeljenje sa 0 nema smisla !
		if (brojac != 0)
	...
	{
		//Provjeravamo da li je trenutni prosjek veci od onog na indeks_najboljeg_Kluba, ako jeste indeks_najboljeg_Kluba poprimi vrijednost trenutnog i;
